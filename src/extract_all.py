...
def extract_kpi_data(company):
    ...
    # Extract company-level fields for fallback (latest known values)
    ...
    if kpis and isinstance(kpis, list) and len(kpis) > 0:
        for kpi in kpis:
            ...
            kpi_ca_bilan = kpi.get('ca_bilan')
            kpi_resultat_exploitation = kpi.get('resultat_exploitation')
            kpi_resultat_bilan = kpi.get('resultat_bilan')
            kpi_fonds_propres = kpi.get('fonds_propres')
            kpi_date_cloture = kpi.get('date_cloture_exercice')
            ...
            record = {
                ...
                'fonds_propres': float(kpi_fonds_propres) if kpi_fonds_propres is not None else (float(fonds_propres) if fonds_propres is not None else None),
                'ca_france': None,
                'date_cloture_exercice': kpi_date_cloture if kpi_date_cloture else (date_cloture if date_cloture else None),
                'duree_exercice': None,
                'salaires_traitements': None,
                'charges_financieres': None,
                'impots_taxes': None,
                'ca_bilan': float(kpi_ca_bilan) if kpi_ca_bilan is not None else (float(ca_bilan) if ca_bilan is not None else None),
                'resultat_exploitation': float(kpi_resultat_exploitation) if kpi_resultat_exploitation is not None else (float(resultat_exploitation) if resultat_exploitation is not None else None),
                'dotations_amortissements': None,
                'capital_social': None,
                'code_confidentialite': None,
                'resultat_bilan': float(kpi_resultat_bilan) if kpi_resultat_bilan is not None else (float(resultat_net) if resultat_net is not None else None),
...
def save_extraction(output_file, records, expected_columns, dataset_name):
    ...
    # Add missing columns in one shot to avoid fragmentation warnings
